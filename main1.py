...
def main():
  ...
  if st.button("bajar todas las notas calificaciones de una materia en xlsx"):
    ...
    docs1 = student_ref.where("materia", "==", materia).where("calificacion0", "array_contains", "0").to_dict().get()
    # Second query for students with calificacion1 of 0
    docs2 = student_ref.where("materia", "==", materia).where("calificacion1", "array_contains", "0").to_dict().get()
    # Third query for students with calificacion2 of 0
    docs3 = student_ref.where("materia", "==", materia).where("calificacion2", "array_contains", "0").to_dict().get()
    # Fourth query for students with calificacion3 of 0
    docs4 = student_ref.where("materia", "==", materia).where("calificacion3", "array_contains", "0").to_dict().get()
    # Fifth query for students with calificacion4 of 0
    docs5 = student_ref.where("materia", "==", materia).where("calificacion4", "array_contains", "0").to_dict().get()
    st.write(docs1)
